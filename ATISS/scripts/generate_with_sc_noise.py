...
def transform_shape_codes(shape_codes):
    """Add some noise to items""" 
    shape = shape_codes.shape
    noise_type = input("Please enter the noise sampling from [uniform, gaussian]\n")
    noise_type = noise_type.lower()
    if noise_type == "uniform":
        msg = "Please enter a range a,b \n"
        range = [int(val) for val in input(msg).split(",") if val != ""]
        noise =  torch.FloatTensor(shape).uniform_(range[0],range[1])
    if noise_type == "gaussian":
        msg = "Please enter mean and standard dev\n"
        vars = [int(val) for val in input(msg).split(",") if val != ""]
        noise = torch.normal(vars[0] * torch.zeros(shape),vars[1] * torch.ones(shape))
    else:
        # Default to uniform
        noise = torch.FloatTensor(shape).uniform_(-1,1)
    return noise + shape_codes
...
def main(argv):
    ...
    for i in range(args.n_sequences):
        ...
        new_shape_codes = transform_shape_codes(boxes["shape_codes"])
        voxel_shapes_t = autoencoder.decoder(torch.squeeze(new_shape_codes))
