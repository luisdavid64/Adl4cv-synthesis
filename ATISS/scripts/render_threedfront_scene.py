...
def main(argv):
    ...
    parser.add_argument(
        "--with_floor_layout",
        action="store_true",
        help="Visualize also the room's floor"
    )
    parser.add_argument(
        "--with_walls",
        action="store_true",
        help="Visualize also the room's walls"
    )
    parser.add_argument(
    ...
    for s in d.scenes:
        if s.scene_id == args.scene_id:
            ...
            renderables = s.furniture_renderables(
                with_floor_plan_offset=True, with_texture=args.with_texture, with_origin=True, with_bboxes=True, with_bbox_corners=True, with_floor_plan=True
            )
            trimesh_meshes = []
